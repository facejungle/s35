...
export function checkLinkType(link: linkType) {
   if (link.__component === 'link.page-static') {
      if (link.slug === 'front page') {
         return {
            linkUrl: '/',
            linkText: link.text
         }
      }
      if (link.slug === 'contacts') {
         return {
            linkUrl: '/kontakty',
            linkText: link.text
         }
      }
      if (link.slug === 'portfolio') {
         return {
            linkUrl: '/portfolio',
            linkText: link.text
         }
      }
      if (link.slug === 'projects') {
         return {
            linkUrl: '/proekty',
            linkText: link.text
         }
      ...
      if (link.slug === 'prices - house') {
         return {
            linkUrl: '/tseny/doma-bani',
            linkText: link.text
         }
      }
      if (link.slug === 'prices - service') {
         return {
            linkUrl: '/tseny/uslugi',
            linkText: link.text
         }
      }
      if (link.slug === 'prices - delivery') {
         return {
            linkUrl: '/tseny/dostavka',
            linkText: link.text
         }
      }
   }
